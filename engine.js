...
// Close button state for settings panel
window.closeBtnActive = false;

// Handle close button click for settings panel
canvas.addEventListener('mousedown', function(evt) {
    if (!showSettingsPanel || !window.closeBtnRegion) return;
    const rect = canvas.getBoundingClientRect();
    const mx = evt.clientX - rect.left;
    const my = evt.clientY - rect.top;
    const btn = window.closeBtnRegion;
...
function init() {
    ...
    // === Play title music on mouse movement ===
    let titleMusicPlayed = false;
...
function updatePlayer(dt) {
    ...
    for (let b of barricades) {
        ...
        // Only block movement, no glow or sound
...
function drawSettingsPanel() {
    ...
    // Responsive plate size and position (wider for sliders)
    const plateWidth = Math.max(600, canvas.width * 0.7);
    const plateHeight = Math.max(400, canvas.height * 0.6);
    const plateX = (canvas.width - plateWidth) / 2;
    const plateY = (canvas.height - plateHeight) / 2;
    // Plate background with rounded corners and reduced shadow
    ...
    const btnW = 48, btnH = 48;
    const btnX = plateX + 32;
    const btnY = plateY + plateHeight - btnH - 32;
    ...
    ctx.textAlign = "left";
    ctx.fillText("SETTINGS", plateX + 40, plateY + 60);
    // Sliders (even smaller width and height)
    const sliderStartY = plateY + 120;
    const sliderWidth = Math.min(180, plateWidth - 160);
    drawGoldSlider("Music", window.volumeMusic || 0.7, sliderStartY, plateX + 60, sliderWidth, 'volumeMusic');
    drawGoldSlider("SFX", window.volumeSFX || 1.0, sliderStartY + 40, plateX + 60, sliderWidth, 'volumeSFX');
    drawGoldSlider("Voices", window.volumeVoices || 1.0, sliderStartY + 80, plateX + 60, sliderWidth, 'volumeVoices');
    // CONTROLS header
    const controlsX = plateX + plateWidth / 2 + 20;
    ctx.fillStyle = "white";
    ctx.font = "32px Arial Black";
    ctx.textAlign = "left";
    ctx.fillText("CONTROLS", controlsX, plateY + 60);
    // Controls list
    ctx.fillStyle = "#00FF00";
    ctx.font = "18px Arial";
    const controlsListStartY = plateY + 120;
    const controls = [
        "← / → : Move left/right",
        "↑ : Block",
        "↓ : Melee attack",
    ...
    for (let i = 0; i < controls.length; i++) {
        ctx.fillText(controls[i], controlsX + 20, controlsListStartY + i * 28);
    }
    ctx.restore();
}
...
function drawGoldSlider(label, value, y, x, width, setter) {
    const height = 8;
    // Draw label
    ctx.font = "16px Arial";
    ctx.fillStyle = "white";
    ctx.fillText(`${label}: ${(value * 100).toFixed(0)}%`, x, y - 6);
    // Draw gold bar
    ...
    ctx.beginPath();
    ctx.arc(knobX, y + height / 2, 6, 0, Math.PI * 2);
    ctx.fillStyle = '#FFD700';
    ctx.fill();
    ctx.strokeStyle = '#FFF';
    ctx.lineWidth = 2;
    ctx.stroke();
    // Register slider region for mouse interaction (optional, if you want to add drag logic)
}
// (This stray code block is removed; see below for correct usage in drawEnemies)
...
function drawHUD() {
    ...
    function drawHUD() {
        ...
        // Center HUD horizontally at top, meters side by side
        const totalMeters = 3;
        const meterSpacing = 120;
    ...
    function drawCircularMeter({ctx, centerX, centerY, radius, thickness, percent, label, value, color, gradient}) {
        // Background circle
        ctx.beginPath();
        ...
        } else if (images.coin && images.coin.complete) {
            ctx.drawImage(images.coin, centerX - 16, centerY - 16, 32, 32);
        }
        // Value text below circle
        ctx.font = 'bold 14px Impact, Arial Black, sans-serif';
        ctx.textAlign = 'center';
        ctx.textBaseline = 'middle';
        // Gradient for value text
        const grad = ctx.createLinearGradient(centerX - 30, centerY + radius + 10, centerX + 30, centerY + radius + 20);
        const stops = gradient(percent);
        grad.addColorStop(0, stops[0]);
        grad.addColorStop(1, stops[1]);
        ctx.fillStyle = grad;
        ctx.fillText(value, centerX, centerY + radius + 20);
    }
    // === Game Screens
    if (gameState === "title") return drawTitleScreen();
    if (gameState === "settings") return drawSettingsPanel();
...
// === End Screen Buttons ===
function drawEndScreenButton(text = "Next Level", onClick = null) {
    const buttonWidth = 200;
    const buttonHeight = 80;
    const startX = (canvas.width - buttonWidth) / 2;
    const startY = canvas.height - buttonHeight - 50;
